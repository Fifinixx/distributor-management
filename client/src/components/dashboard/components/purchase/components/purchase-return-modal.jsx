...
export function ReturnDialog({ order, fetchOrders }) {
  ...
  async function processReturn() {
    try {
      setLoading(true);
      const res = await fetch(`${baseUrl}/dashboard/orders/return`, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button disabled onClick={() => setOpen(true)} variant="outline">
          <IoIosReturnLeft />
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        ...
        <DialogFooter>
          ...
          <Button onClick={async () => {await processReturn(); setOpen(false); fetchOrders()}} disabled={loading} type="submit">
